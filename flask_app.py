...
def handle_dialog(req, res, anim='слона'):
    ...
    if req['session']['new']:
        ...
        res['response']['text'] = f'Привет! Купи {anim}!'
        # Получим подсказки
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        res['response']['text'] = f'{anim.title()} можно найти на Яндекс.Маркете!'
        handle_dialog(req, res, 'кролика')
        return

    # Если нет, то убеждаем его купить слона!
    res['response']['text'] = \
        f"Все говорят '{req['request']['original_utterance']}', а ты купи {anim}!"
    res['response']['buttons'] = get_suggests(user_id, anim=anim)


# Функция возвращает две подсказки для ответа.
def get_suggests(user_id, anim='слон'):
    ...
    if len(suggests) < 2:
        suggests.append({
            "title": "Ладно",
            "url": f"https://market.yandex.ru/search?text={anim}",
            "hide": True
        })
